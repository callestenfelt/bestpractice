...
**Session 16 — daily SQLite backup via systemd timer (2026-05-18).** `scripts/backup_db.sh` snapshots `/opt/bestpractice/data/bestpractice.db` to `/var/backups/bestpractice/bestpractice-<UTC-stamp>.db.gz` once a day, then prunes anything older than 14 days. Uses `sqlite3 .backup` (lock-safe, no need to stop the Flask service) + `PRAGMA integrity_check` smoke test + atomic rename + gzip. Fires from `bestpractice-backup.timer` at 03:17 UTC daily (`Persistent=true` catches missed runs). Service runs as root since the DB is root-owned; `Nice=10` + `IOSchedulingClass=idle` keep it polite. One-time VPS install steps live in `nextstep.md` Session 16 — the script rsyncs in via GHA but the `.service`/`.timer` files need a one-shot copy to `/etc/systemd/system/` + `systemctl enable --now bestpractice-backup.timer`. Override paths via env vars (`BESTPRACTICE_DB`, `BESTPRACTICE_BACKUP_DIR`, `BESTPRACTICE_BACKUP_RETAIN_DAYS`) if needed.

Still pending: `/admin/considerations/<slug>` editor, MDN browser-compat-data adapter, per-source-type score threshold, `/admin/sources` UX polish (error display, config_json editor), content-diff for structured sources. See `nextstep.md` Session 16 pointer.
